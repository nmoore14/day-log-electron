<template>
	<div class="flex-col w-full h-full">
		<div class="flex-1 bg-grey-light h-48 rounded items-center justify-center">
			<h1 class="w-full font-sans font-light text-center text-2xl text-grey-darkest">Enter a New Todo:</h1>
			<input type=text placeholder="Todo Title" class="flex-1 w-2/5 bg-grey-lightest font-sans font-thin text-xl text-center rounded p-2 ml-68 shadow-md" v-model="newTodo.title">
			<textarea
        name="notes_home"
        id="notes-home"
        cols="50"
      ...
      ></textarea>
			<button class="w-96 ml-66 mt-1 text-white font-thin py-2 px-4 border border-grey-darkest rounded hover:shadow-md hover:border-grey-lightest" id="todo-submit-btn" @click="addTodo()">
				Add
			</button>
		</div>
...
<style scoped>
	#todo-submit-btn {
		background: linear-gradient(90deg, #2196F3, #8E24AA);
	}

	#todo-submit-btn:hover {
		background: linear-gradient(90deg, #135A91, #471255);
	}
</style>
